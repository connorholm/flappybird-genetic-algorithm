...
def fitness_func(ga_instance, solution, solution_idx):
    ...
    if (solution < pipeHeight + nearest_upper_pipe[
        'y'] + 70):  # and abs(playerx - nearest_upper_pipe['x']) < GAME_SPRITES['pipe'][0].get_width() + 30):
    ...
    if (solution + GAME_SPRITES['player'].get_height() > nearest_lower_pipe[
        'y'] - 50):  # and abs(playerx - nearest_lower_pipe['x']) < GAME_SPRITES['pipe'][0].get_width() + 30:
...
if __name__ == "__main__":
    ...
    GAME_SPRITES['message'] = pygame.transform.scale(pygame.image.load('gallery/sprites/pygad.png').convert_alpha(),
                                                     [400, 300])
